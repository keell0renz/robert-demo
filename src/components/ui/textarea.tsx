  ...
  return (
    <textarea
      className={cn(
        [
          // base
          "text-paragraph-sm text-foreground block w-full resize-none outline-none",
          !simple && ["pointer-events-auto h-full min-h-[82px] bg-transparent pt-2.5 pr-2.5 pl-3"],
          simple && [
            "bg-card shadow-regular-xs min-h-28 rounded-xl px-3 py-2.5",
            "ring-border ring-1 ring-inset",
            "transition duration-200 ease-out",
            // hover
            "hover:[&:not(:focus)]:bg-muted",
            !hasError && [
              // hover
              "hover:[&:not(:focus)]:ring-transparent",
              // focus
              "focus:shadow-button-important-focus focus:ring-ring",
            ],
            hasError && [
              // base
              "ring-error-base",
            ...
            disabled && ["bg-muted ring-transparent"],
          ],
          !disabled && [
            // placeholder
            "placeholder:text-foreground-soft placeholder:transition placeholder:duration-200 placeholder:ease-out placeholder:select-none",
            // hover placeholder
            "group-hover/textarea:placeholder:text-muted-foreground",
            // focus
            "focus:outline-none",
            // focus placeholder
          ...
          disabled && [
            // disabled
            "text-foreground-disabled placeholder:text-foreground-disabled",
          ],
        ],
        className,
      )}
  ...
  return (
    <div
      className={cn(
        [
          // base
          "group/textarea bg-card shadow-regular-xs relative flex w-full flex-col rounded-xl pb-2.5",
          "ring-border ring-1 ring-inset",
          "transition duration-200 ease-out",
          // hover
          "hover:[&:not(:focus-within)]:bg-muted",
          // disabled
          "has-[[disabled]]:bg-muted has-[[disabled]]:pointer-events-none has-[[disabled]]:ring-transparent",
        ],
        !hasError && [
          // hover
          "hover:[&:not(:focus-within)]:ring-transparent",
          // focus
          "focus-within:shadow-button-important-focus focus-within:ring-ring",
        ],
        hasError && [
          // base
          "ring-error-base",
...
function CharCounter({
  ...
  return (
    <span
      className={cn(
        "text-subheading-2xs text-foreground-soft",
        // disabled
        "group-has-[[disabled]]/textarea:text-foreground-disabled",
        {
